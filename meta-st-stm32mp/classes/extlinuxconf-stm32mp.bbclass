...
def create_extlinux_file(cfile, labels, data):
    ...
    bb.warn("TTTT create_extlinux_file  START cfile: '%s'  "% cfile)
    try:
...
python do_create_multiextlinux_config() {
    ...
    bb.warn("TTTT do_create_multiextlinux_config START ")
    ...
    for target in targets.split():
        bb.warn("Loop for '%s' target" % target)
        ...
        if fit_config == '1':
            ...
            bb.warn("Create %s/extlinux.conf file for %s labels" % (subdir, labels))
        ...
        if len(extra_extlinuxtargetconfig.split()) > 0:
            ...
            for config in extra_extlinuxtargetconfig.split():
                ...
                bb.warn("TTT UBOOT_EXTLINUX_TARGETS_EXTRA_CONFIG config: '%s' " % config)
